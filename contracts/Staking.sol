...
contract Staking is Ownable {
    ...
    constructor (address _KEEPER, address _sKEEPER, uint32 _epochLength, uint32 _firstEpochNumber, uint32 _firstEpochTime) {
    ...
    function claim ( address _recipient ) public returns ( uint ) {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            return _send( _recipient, sKEEPER.balanceForGons( info.gons ));
        }
        return 0;
    }
